...
from app.auth import verify_api_key_flexible, get_current_api_key
from app.config import get_settings
from app.database import DatabaseService
from app.services.ipfs import IPFSService
from app.logging_config import get_logger, get_log_files, read_log_file, log_system_event
...
async def get_log_content(
    ...
    try:
        ...
        raise HTTPException(status_code=403, detail=f"Permission denied reading '{filename}'")
    except Exception as e:
...
async def clear_logs(
    log_type: str = Query(..., regex="^(all|access|errors|main)$", description="Type of logs to clear"),
    api_key: str = Depends(verify_api_key_flexible)
):
    """
