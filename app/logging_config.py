...
def log_api_access(
    ...
    access_logger.info(
        ...
        f"{response_time:.3f}s | {user_agent or 'Unknown'}"
    )
...
def log_system_event(event: str, details: str | None = None, level: str = "INFO"):
    """Log system events."""
...
def read_log_file(filename: str, lines: int = 100, tail: bool = True) -> list:
    ...
    try:
        ...
        msg = f"Error reading log file {filename}: {str(e)}"
