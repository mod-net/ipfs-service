...
class IPFSService:
    ...
    async def get_node_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            node_info = await loop.run_in_executor(None, client.id)
            return node_info
        except Exception as e:
            raise HTTPException(
                status_code=503,
    ...
    async def add_file(self, file: UploadFile) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await loop.run_in_executor(
                None,
                lambda: client.add(io.BytesIO(content), pin=True)
            )
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to add file to IPFS: {str(e)}"
            )
    ...
    async def list_pinned_files(self) -> list[str]:
        ...
        try:
            ...
            pinned = await loop.run_in_executor(
                None,
                lambda: client.pin.ls(type='recursive')
            )
