...
class DirectoryMapper:
    def __init__(
        ...
        self.content_extensions = (
            content_extensions  # Allow None to include all extensions
        )
            ...
            if self.content_extensions is None or file_ext in self.content_extensions:
                ...
                try:
                    with open(file_path, encoding="utf-8") as f:
                        ...
                        print(
                            f"Successfully read {len(content)} bytes from {file_path}"
                        )
                except Exception as e:
                    print(f"Warning: Could not read content of {file_path}: {e}")
            else:
                print(
                    f"Skipping content for {file_path} - extension not in allowed list"
                )

        return node

    def map_directory(self, dir_path: str, depth: int = 0) -> dict[str, Any]:
        ...
        if depth >= self.max_depth:
            return {
                ...
                "path": dir_path,
                "children": {
                    "[MAX_DEPTH_REACHED]": {"type": "file", "name": "", "path": ""}
                },
            }
    ...
    def generate_markdown(self, node: dict[str, Any], level: int = 0) -> str:
    ...
    def save_map(
        self, node: dict[str, Any], output_path: str, file_format: str = "json"
    ):
...
def main():
    ...
    parser.add_argument(
        "--include-content",
        action="store_true",
        help="Include file contents in the map",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--content-extensions",
        nargs="*",
        help="File extensions to include content from (default: all if --include-content)",
    )
