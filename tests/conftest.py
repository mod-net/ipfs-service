...
from app.database import Base, get_db, DatabaseService, FileRecord
...
def mock_ipfs_service():
    ...
    # Mock async methods
    mock_service.get_node_info = AsyncMock(return_value={
    ...
    mock_service.get_file_stats = AsyncMock(return_value={
        "DataSize": 1024,
        "CumulativeSize": 1024,
        "NumLinks": 0,
        "Type": "file"
    ...
    # Mock sync methods
    mock_service.get_gateway_url = Mock(return_value="http://localhost:8080/ipfs/QmTestCID123456789")
...
def test_client(mock_ipfs_service, test_db):
    ...
    from app.api.files import get_ipfs_service, get_database_service
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "ipfs: marks tests that require IPFS node"
    )
