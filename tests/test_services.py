...
class TestIPFSService:
    ...
    async def test_check_connection_failure(self, ipfs_service):
        ...
        with patch.object(
            ipfs_service, "get_node_info", side_effect=Exception("Connection failed")
        ):
            result = await ipfs_service.check_connection()
...
class TestServiceIntegration:
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_file_deletion_integration(
        self, mock_ipfs_service, db_service, sample_file_record
    ):
