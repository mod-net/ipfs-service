...
class VersionManager:
    ...
    def get_current_version(self) -> Tuple[int, int, int]:
        ...
        try:
            ...
            if match:
                ...
                self._print_warning(f"Invalid version format in {self.version_file}: {version_str}")
                return (0, 1, 0)
    ...
    def bump_version(self, bump_type: str, changelog_entry: str = None) -> Tuple[int, int, int]:
...
"""
            ...
            if "## [Unreleased]" in content:
                ...
                if len(parts) == 2:
                    ...
                    if next_section_match:
                        ...
                        # Insert at the end
                        changelog_content = parts[0] + "## [Unreleased]" + after_unreleased + new_entry
                else:
    ...
    def create_git_tag(self, major: int, minor: int, patch: int, message: str = None):
...
{Colors.WHITE}INTERACTIVE MODE:{Colors.NC}
    ...
    def interactive_mode(self):
        ...
        while True:
            try:
                choice = input(f"\n{Colors.CYAN}Enter your choice (0-6): {Colors.NC}").strip()
                ...
                if choice == '0':
                    ...
                    self._print_error("Invalid choice. Please enter a number between 0-6.")
                    self.show_help()
    ...
    def _handle_interactive_bump(self, bump_type: str):
        ...
        message = input(f"Enter changelog message for {bump_type} bump (optional): ").strip()
        try:
            new_version = self.bump_version(bump_type, message if message else None)
            self._print_success(f"Bumped {bump_type} version to {'.'.join(map(str, new_version))}")
        except Exception as e:
    ...
    def _handle_interactive_set(self):
        ...
        try:
            ...
            if match:
                ...
                self._print_error("Invalid version format. Use major.minor.patch (e.g., 1.2.3)")
        except Exception as e:
...
Examples:
  ...
  python version_manager.py tag "Release message"
    ...
    parser.add_argument('message', nargs='?',
                       help='Changelog entry or tag message')
    ...
    try:
        if not args.command:
            ...
            vm._print_success(f"Bumped {args.type_or_version} version to {'.'.join(map(str, new_version))}")
            ...
            if not match:
                vm._print_error("Invalid version format. Use major.minor.patch (e.g., 1.2.3)")
                sys.exit(1)
